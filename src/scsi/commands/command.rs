...
impl Command {
    pub fn extract_from_cbw(cbw: &CommandBlock) -> Result<Command, Error> {
        ...
        let op_code = OpCode::try_from_primitive(cbw.bytes[0]).map_err(|_| Error::UnhandledOpCode)?;
        match op_code {
            ...
            OpCode::ReadFormatCapacities => {
                Ok(Command::ReadFormatCapacities(overlay(cbw)?))
            }
            ...
            OpCode::Write6 => Ok(Command::Write(
                    (overlay::<Write6Command>(cbw)?).into(),
            )),
            OpCode::Write10 => Ok(Command::Write(
                (overlay::<Write10Command>(cbw)?).into(),
            )),
            OpCode::Write12 => Ok(Command::Write(
                (overlay::<Write12Command>(cbw)?).into(),
            )),
